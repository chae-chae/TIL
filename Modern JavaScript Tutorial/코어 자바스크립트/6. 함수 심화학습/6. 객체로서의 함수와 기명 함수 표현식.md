...
그런데 객체 메서드 이름은 함수처럼 자동 할당이 되지 않는다. 적절한 이름을 추론하는 게 불가능한 상황이 있는데, 이때 name 프로퍼티엔 빈 문자열이 저장된다. 아래와 같이 말이다.
...
대신 `func`과 같은 이름을 붙이면 두 가지가 변화가 생긴다. 이 두 변화 때문에 기명 함수 표현식을 사용하는 것이다.
