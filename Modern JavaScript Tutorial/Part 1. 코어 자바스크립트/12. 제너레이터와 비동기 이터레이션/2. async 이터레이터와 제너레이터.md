...
익숙한 예시인 이터러블 객체 `range`를 토대로, 일초마다 비동기적으로 값을 반환하는 이터러블 객체를 만들어보겠다.
...
1. 다운로드는 [fetch](https://ko.javascript.info/fetch) 메서드로 하겠다. `fetch`를 사용하면 인증 정보나 헤더 등을 함께 실어 요청할 수 있다. GitHub에서 강제하는 `User-Agent`를 헤더에 실어 보내겠다.
