_반복 가능한(iterable, 이터러블)_ 객체는 배열을 일반화한 객체다. 이터러블 이라는 개념을 사용하면 어떤 객체에든 `for..of` 반복문을 적용할 수 있다.
...
인덱스와 `length` 프로퍼티가 있는 객체는 _유사 배열_ 이라 불린다. 유사 배열 객체엔 다양한 프로퍼티와 메서드가 있을 수 있는데 배열 내장 메서드는 없다.
